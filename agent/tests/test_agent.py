...
class TestMyAgentLangGraph:
    ...
    def test_prompt_template_formats_with_variables(self):
        ...
        format_kwargs = {
            "chat_history": "前回は打錠工程について質問しました。",
            "topic": "コーティングのムラを防ぐには？",
        }
        ...
        assert "コーティングのムラ" in messages[1].content

    @patch("agent.myagent.create_agent")
    def test_graph_factory_creates_pfmea_assistant(self, mock_create_agent):
        """Test that graph_factory creates a graph with the pfmea_assistant node."""
        mock_llm = Mock()
        mock_tool = Mock()
        graph = graph_factory(mock_llm, [mock_tool], verbose=False)
        assert graph is not None
        assert "pfmea_assistant" in graph.nodes

    @patch("agent.myagent.create_agent")
    def test_graph_factory_passes_llm_and_tools(self, mock_create_agent):
        """Test that graph_factory passes LLM and external tools to the agent."""
        mock_llm = Mock()
        mock_tool = Mock()
        graph_factory(mock_llm, [mock_tool], verbose=True)
        assert mock_create_agent.call_count == 1
        call = mock_create_agent.call_args_list[0]
        assert call[0][0] == mock_llm
        assert mock_tool in call[1]["tools"]

    @patch("agent.myagent.create_agent")
    def test_graph_factory_includes_pfmea_search(self, mock_create_agent):
        """Test that graph_factory always includes the built-in pfmea_search tool."""
        mock_llm = Mock()
        graph_factory(mock_llm, [], verbose=False)
        call = mock_create_agent.call_args_list[0]
        tool_names = [t.name for t in call[1]["tools"]]
        assert "pfmea_search" in tool_names
    ...
    def test_workflow_property_uses_graph_factory(self, agent):
        ...
        with patch("agent.myagent.create_agent"):
            ...
            assert "pfmea_assistant" in workflow.nodes
