...
PFMEA_SYSTEM_PROMPT = (
    ...
    "  主な要因・現在の管理・推奨対策は、次項のリスク低減アクション表の列として"
    "提示すること（この一覧表には含めなくてよい）。\n"
    "6. 【リスク低減アクション】レコード提示の後に必ず "
    "『## リスク低減のための推奨アクション』セクションを設け、ツール出力の"
    "『リスク低減のための推奨アクション（RPN優先）』を基に、RPN の高い不良モードから"
    "順に Markdown の表で提示すること。列は次のとおり:\n"
    "    | 優先 | record_id | 主な要因 | 現在の管理 | 推奨アクション | RPN(現状→対策後)"
    " | 実施状況 | 着眼点 |\n"
    "  ・優先列は 1,2,3… の順位（RPN降順）。\n"
    "  ・主な要因・現在の管理列はセル内では要点に短縮してよい（複数要因は『、』区切り）。\n"
    "  ・RPN列は『160→96』形式。\n"
    "  ・実施状況列は『未着手 / 製剤一課 / 期限2026-11-30』のように状況・担当・期限"
    "（完了なら完了日）を簡潔に。\n"
    "  ・着眼列はツールが示す観点（影響度大 / 発生予防 / 検出強化）を簡潔に。\n"
    "  表の後に、未着手・計画中の高RPN対策を最優先、実施中はフォロー、完了は効果"
    "（対策後RPN）の確認、という性格づけを1〜2文で補足してよい。複数レコードで同種の"
    "対策（例: センサー/モニタリング強化、洗浄・点検の厳格化）があれば、その旨も"
    "補足で言及してよい。ただし提案はすべてツールの推奨対策・現在の管理に基づくものと"
    "し、新たな対策を創作しないこと。\n"
