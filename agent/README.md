...
4. **RPN 意図検出**: 「RPN が高い」「リスクが高い」等を検出すると RPN 降順モードに切替。トピック語があればそれで絞ってから RPN 降順。
5. **実施状況フィルタ**: 「まだ対策が打たれていない高リスクは？」「実施中の対策は？」等を検出すると、対策の実施状況（未着手/計画中/実施中/完了/見送り）で絞り RPN 降順（残存リスク・進捗管理）。
6. **上位 5 件**を返却。該当なし時は言い換え例を提示。

各レコードは検索結果に加えて、**対策の実施状況**（担当・期限/完了日）と **対策後の想定 RPN（現状→対策後）** を提示します。回答生成では system_prompt（`agent/myagent.py`）で「検索結果に基づくのみ」「record_id を根拠として併記」「未着手の高 RPN を最優先」を指示しています。
